...
Attributes_20y_7trts<-recov_b %>% 
  dplyr::filter(.,age=="20") %>%
  pivot_wider(id_cols=c("chronosequence","age"),
              names_from=attribute,
              values_from=c(recov_kld)) %>% 
  select(.,age, chronosequence, AGB, gini, maxDBH, q0, q2, nfix, WD) %>% 
  drop_na()

Attributes_20y_Alltrts<-recov_b %>% 
  dplyr::filter(.,age=="20") %>%
  pivot_wider(id_cols=c("chronosequence","age"),
              names_from=attribute,
              values_from=c(recov_kld)) %>% 
  select(.,age, chronosequence, AGB, maxDBH, gini, chao, q0, q2, SLA, WD, nfix, C_vol, P_vol, N_vol, soilbd)

save(Attributes_20y_Alltrts, file="Data/MDR_RecoveryRates_All_Attributes.RData")
